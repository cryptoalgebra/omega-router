...
Please read the [Contributions](#contributions) section before submitting a Pull Request.
...
The Omega Router is a universal ERC20 swap router that allows users greater flexibility when performing trades across multiple DEX protocols.
...
The Omega Router codebase consists of the `OmegaRouter` contract and all of its dependencies. The purpose of the `OmegaRouter` is to allow users to unify token swaps across Algebra Integral, Uniswap V3, and Uniswap V2, as well as manage liquidity positions on Algebra Integral, in a single transaction.

`OmegaRouter` integrates with [Permit2](https://github.com/Uniswap/permit2), to enable users to have more safety, flexibility, and control over their ERC20 token approvals.

### OmegaRouter command encoding

Calls to `OmegaRouter.execute`, the entrypoint to the contracts, provide 2 main parameters:
...
The inputs for `INTEGRAL_SWAP_EXACT_IN` is the encoding of 5 parameters:

- `address` The recipient of the output of the trade
- `uint256` The amount of input tokens for the trade
- `uint256` The minimum amount of output tokens the user wants
- `bytes` The Algebra Integral path you want to trade along
- `bool` A flag for whether the input funds should come from the caller (through Permit2) or whether the funds are already in the OmegaRouter
...
```
git clone --recurse-submodules https://github.com/cryptoalgebra/omega-router.git
```
...
1. Install the latest version of the Omega Router package.
2. Add git submodules for contracts that aren't a node package. Make sure there's an empty `.gitmodules` file. Then run:
    ...
    ```
3. You should now be able to import contracts from omega-router and compile.
...
#### To Deploy Permit2 Alongside OmegaRouter
...
script/deployParameters/DeployOmegaRouter.s.sol:DeployOmegaRouter \
<pathToJSON>
```
