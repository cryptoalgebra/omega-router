...
abstract contract V2Quoter is UniswapImmutables {
    ...
    function v2QuoteExactInput(
        uint256 amountIn,
        address[] calldata path
    ) internal view returns (uint256 amountOut) {
        amountOut = UniswapV2Library.getAmountOutMultihop(
            UNISWAP_V2_FACTORY, UNISWAP_V2_PAIR_INIT_CODE_HASH, amountIn, path
        );
    }
    ...
    function v2QuoteExactOutput(
        uint256 amountOut,
        address[] calldata path
    ) internal view returns (uint256 amountIn) {
        (amountIn,) = UniswapV2Library.getAmountInMultihop(
